...
try:
	...
	requestsinstallstatus = fullpath = input ('Requests module is missing, would you like to automatically install? (Y/N): ')
...
try: 
	...
	ciscoconfparseinstallstatus = fullpath = input ('CiscoConfParse module is missing, would you like to automatically install? (Y/N): ')
...
fullpath = input ('Enter the file path of the ASA config: ')
importnetworkobjectsq = input ('Do you want to import Network Objects? (Y/N): ')
importserviceobjectsq = input ('Do you want to import Port Objects? (Y/N): ')
'''importaclq = input ('Do you want to import ACL rules? IN DEVELOPMENT (Y/N): ')'''
importaclq = 'N'
...
fullpath = input ('Enter the file path of the ASA config: ')
fmcpathfull = input ('Enter the IP address of the destination FMC: ')
fmcpath = "https://" + fmcpathfull
fmcuser = input ('Enter the username of the destination FMC: ')
...
'''NOT IMPLEMENTED'''
'''importnatq = input ('Do you want to import NAT rules (Y/N): ')'''
		...
		for objectservicechild in specificobjectservice.children:
			...
			if re.match(r"\d+",objectservicechild_protocol) :
				...
				if re.match(r"^[a-zA-Z]*$",objectservicechild_port):
					...
					try:
						csvdownload = urllib3.urlopen(csvpath)
